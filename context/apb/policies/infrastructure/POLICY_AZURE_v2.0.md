# Políticas Azure - Gobernanza con Azure Policy y Etiquetas v2.0
...
- Plan de control aplicable a Management Groups (MG), todas las suscripciones y recursos (Core, Preproducción, Producción).
...
Matriz por categorías de políticas: Seguridad y Cumplimiento, Gobernanza y organización, Monitorización y auditoría, FinOps.

La matriz RACI define las responsabilidades de los distintos equipos en relación con la definición, cumplimiento, supervisión y gestión de excepciones de las Políticas IT:
...
Se etiquetará en inglés y todo en minúsculas. Adicionalmente existe un documento donde se detalla la lista de etiquetas a aplicar en Azure con más información de cada una. A modo de resumen:
...
Las etiquetas están organizadas por tags, con descripciones, nivel de aplicación y responsables.
...
La estrategia para aplicar las políticas para minimizar un impacto negativo en la organización será por fases. La lista de fases y ejemplos de aplicación por faseado se definirán en el proyecto de implementación.
...
La lista de recomendaciones de efecto por categoría de política se definirá en el proyecto de implementación, alineada con el catálogo de políticas aprobado.

## Anexo 1 - Propuesta de nomenclatura
...
Decidir si suscripción en alguna posición, guiones o no y en cuáles, minúsculas y en inglés.

### Significado de los bloques

- `apb`: prefijo organizativo fijo.
- `<app/proyecto>`: identificador de aplicación o proyecto.
- `<tiporecurso>`: abreviatura del tipo de recurso.
- `<entorno>`: dev / pre / pro / tst.
- `<región>`: abreviatura de región (es / fr / etc.).
- `<número+opcional>`: sufijo numérico cuando hay múltiples instancias.

### Abreviaturas de región (recomendadas)

Revisar nombre oficial en la documentación de Azure.

### Abreviaturas de tipos de recursos

La lista de abreviaturas de tipos de recursos se mantendrá en el documento de referencia del proyecto de implementación.

### Ejemplos de nombres por tipo de recurso

```
Resource Group:     apb-mps-rg-pro-es
App Service:        apb-mps-app-pre-es
Storage Account:    apbmpsstproes01
Key Vault:          apb-mps-kv-pro-fr
Virtual Network:    apb-hub-vnet-pro-es
Subred:             apb-hub-snet-firewall-pro-es
Public IP:          apb-portal-pip-dev-es-01
```

### Ejemplo completo de arquitectura usando el estándar APB
