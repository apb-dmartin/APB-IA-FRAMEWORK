...
| DDD/#38 F0 | Análisis de dominios sobre inventario APIM → poblar APB-DOMAIN-CATALOG | ✅ **Completa (2026-06-30)** — `API_INVENTORY_APIM.md` (~118 APIs) → 15 dominios de negocio `proposed` en `APB-DOMAIN-CATALOG/domains/`. Artefactos en `ddd-discovery/`. **Pendiente:** PR + aprobación de Arquitectura; fronteras de negocio (Escalas/Cruceros/Líneas, INS/PIF, MDE/OPS, FER). **Diferido:** bounded contexts (requiere acceso a código/BBDD). |
